...
reader()
{
  ...
  if (fdefines) /* bug fix by Dan -- for link-checking, must extern this! */
    ...
    if (yylsp_needed)
      fprintf(fdefines, LTYPESTR_EXTERN);
  ...
  while (count > 0)
    {
      while (c != '}')
	      ...
	      while (!ended)
		{
	      ...
	      break;
	      /* JGM: we use ^$ to abbreviate ^YYSTYPE.<typename> where
	       * <typename> is the union field name that's associated with
		...
		if (type_name)
		  fprintf(faction,"^YYSTYPE.%s",type_name);
		else
	      ...
	      if (c == '$')
		{
		   ...
		   * to use $$ as an r-value, then you'll need to cast it
		   * explicitly (e.g., $$.foo).
		   */
	      ...
	      else
		{
	      ...
	      if (m == 0)
		fprintf(faction, "yyls[yylsp_offset]");
	      else if (m > 0) 
		fprintf(faction, "yyls[yylsp_offset+%d]", m);
