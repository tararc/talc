...
// This is now in the Makefile where it belongs.
// #define XPFILE "c:\\home\\danieljg\\greg_bison\\share\\pop_bison.simple"
// #define XPFILE1 "pop_bison.hairy"
...
openfiles()
{
  ...
  if (spec_outfile)
    {
	 ...
	 For other output files, remove the ".pop" suffix.  */
      name_base = spec_outfile;
#ifdef MSDOS
      strlwr (name_base);
#endif /* MSDOS */
      /* BASE_LENGTH includes "_tab" but not ".pop".  */
      base_length = strlen (name_base);
      if (!strcmp (name_base + base_length - 4, ".pop"))
	base_length -= 4;
      /* SHORT_BASE_LENGTH includes neither "_tab" nor ".pop".  */
  ...
  if (spec_outfile)
    ...
    tabfile = stringappend(name_base, base_length, ".pop");
