...
(* Hacked by Dan for Popcorn -- changed file extensions *)
...
(* $Id: main.ml,v 1.1 1999/04/09 14:00:07 danieljg Exp $ *)
...
let main () =
  ...
  let dest_name =
    if Filename.check_suffix source_name ".popl" then
      Filename.chop_suffix source_name ".popl" ^ ".pop"
    else
      source_name ^ ".pop" in
