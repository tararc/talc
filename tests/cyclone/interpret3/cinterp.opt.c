...
void c_interpret_opt(struct inst prg[], int pc, int sp)
{ 
  /*
	  ...
	  if(prg[pc].opcode == ADD)
		...
		if(prg[pc].opcode == LD)
		  ...
		  if(prg[pc].opcode == ST)
			...
			{
			  if(stack[sp])
			    { pc = prg[pc].val; sp = sp - 1 ; c_interpret_opt(prg,pc, sp); } 
			  else
			    { pc = pc + 1; sp = sp - 1; c_interpret_opt(prg, pc, sp); } 
			}
		      else
