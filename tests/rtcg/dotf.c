...
#ifdef POP
int dot (ARR_TYP(float,u), ARR_TYP(float,v)) {
#else
  int dot (int len, ARR_TYP(float,u), ARR_TYP(float,v)) {
#endif
...
int main(int argc, char **argv) {
  ...
  { 
    ...
    MAIN_TEST("dotf",C_COMPILER,PONLY(POPC_KIND) CONLY(PLAIN_KIND),
	      args,0,DOT(vec_len,u,u));
    
    result = DOT(vec_len,u,u);  
    printf("vec_len = %d, result = %g\n",vec_len, result);
  }
