...
#ident "$Id: coff_obj_i386.c,v 1.1 1999/10/19 18:25:00 mwh Exp $"
...
arch_apply_relocation (struct obj_file *f,
		       ...
		       unsigned int v)
{
  unsigned int *loc = (unsigned int *)(targsec->contents + rel->r_vaddr);
  unsigned int dot = targsec->header->s_vaddr + rel->r_vaddr;
  ...
  switch (rel->r_type)
    ...
    case RELOC_REL32:
      ...
      printf("pc-rel reloc v-dot = %x to @%x, oldval = %x, ", 
	     v - dot, loc, *loc);
#endif
      *loc += v - dot - 4; /* must subtract 4 since PC-rel jumps
                              are from PC + 4 */
#ifdef VERBOSE
      printf("newval = %x\n",*loc);
#endif
