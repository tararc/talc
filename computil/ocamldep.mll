...
(* $Id: ocamldep.mll,v 1.1 1999/02/07 20:00:48 danieljg Exp $ *)
(* Modified by Neal Glew 21 Nov 98 - added unix flag and module pattern *)
...
let find_in_path path name =
  if not (Filename.is_implicit name) then
    ...
    | dir::rem ->
        let fullname = 
          if !unix then dir ^ "/" ^ name 
          else Filename.concat dir name in
...
let (depends_on, escaped_eol) =
  ...
  | "Unix" | "Win32" -> (": ", "\\\n    ")
  | "MacOS" -> ("\196 ", "\182\n    ")
  | x -> fatal_error ("Ocamldep: unknown system type" ^ x)
;;
...
let _ =
  Arg.parse [
     ...
     "--unix",Arg.Set unix,"use unix filenames";
